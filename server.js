...
app.get('/trackAnalysis', function (req, res) {
	...
	request.post(authOptions, function (error, response, body) {
		if (!error && response.statusCode === 200) {
			...
			var parsedQueryString = parse(queryString);

			request.get({
				url: 'https://api.spotify.com/v1/search?q=' + parsedQueryString + '&type=track',
				headers: {
				...
				if (items.length <= 0) {
					res.status(404).end('No tracks with name "' + parsedQueryString + '" found.');
					return;
				}
				var track = items[0];
...
function parse(queryString) {
	var parsedQueryString = queryString;
	if (parsedQueryString.match((/(\(|\[).*remix.*(\)|\])/gi))) { // if we have (..remix...) / [..remix..] we should not delete everything
		parsedQueryString = parsedQueryString.replace(/(\(|\[|\)|\])/gi, ""); // remove the brackets only
	} else {
